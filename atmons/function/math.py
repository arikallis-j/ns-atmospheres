"""
Description of basic math functions
"""
import numpy as np

# Basic functions from Numpy
def sqrt(x):
    return np.sqrt(x)
...
# Specific functions for calculating
def hs(x):
...
def inverse(f, y0, args=(), pw=5, base=0.0):
    """Finding x: f(x) = y0 with accuracy 10^-5 from x_0 = base"""
    eps = 10.0**(-pw) * y0.unit
    h = 1.0 * y0.unit
    base *= y0.unit
...
def map1(X, F, Y, label_flag=True):   
    ...
    for j in range(X.shape[1]):
        ...
        if R[j] < 0 and label_flag:
            if min_X > Y[j] or max_X < Y[j]:
                print(f"{min_X} | {Y[j]} | {max_X}" , label)
    return R.reshape(*p_shape, *g_shape)