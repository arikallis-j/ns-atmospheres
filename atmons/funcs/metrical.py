...
def chi_metric(R: Q[CM], M: Q[GRAM]) -> Q[DIMLESS]:
    """Calculate chi for the neutron star metric (II.eq.A.1))"""
    chi = G_GRAV * M / (R * C_LIGHT**2) #dze
    return chi << DIMLESS

def Omega_metric(R: Q[CM], M: Q[GRAM], omega_rot: Q[SEC**(-1)]) -> Q[DIMLESS]:
    """Calculate Omega for the neutron star metric (II.eq.A.1)"""
    Omega = omega_rot * sqrt(R**3 / (G_GRAV * M)) #epsm
    return Omega << DIMLESS

# Relative q_c and b_c calculating
def q_c_metric(chi: Q[DIMLESS], Omega: Q[DIMLESS]) -> Q[DIMLESS]:
    """Calculate q_c for the neutron star metric (II.eq.A.8)"""
    q_c = -0.11 * (Omega / chi)**2 #q
    return q_c << DIMLESS

def b_c_metric(chi: Q[DIMLESS], Omega: Q[DIMLESS]) -> Q[DIMLESS]:
    """Calculate b_c for the neutron star metric (II.eq.A.9)"""
    b_c = 0.4454 * Omega**2 * chi #bet
    return b_c << DIMLESS

# Relative i_bar calculating
def i_bar_metric(chi: Q[DIMLESS]) -> Q[DIMLESS]:
...
# Radius dependence
def R_metric(R_eq, theta, chi, Omega):
    Omega2 = Omega**2
