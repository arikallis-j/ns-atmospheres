...
class BaseGeff(Experiment):
    def __init__(self, name, **param):
        ...
        self.const = Const(w_func="vkpower-2.0",  w_par = 0.7, th_star=[90])
    ...
    def do_experiment(self,
        ...
        ax.plot(grid.cos_th[0,:l_cos_th-1:], surf.g_th[0,:l_cos_th-1:], color='black', label=f'spread layer')
        ...
        l_cos_th = len(grid.cos_th[0, ::]) // 2
        ax.plot(grid.cos_th[0,:l_cos_th-1:], surf.g_th[0,:l_cos_th-1:], color='blue', label=f'base, v_rot = {v_rot} Hz')
        ...
        ax.plot(grid.cos_th[0,:l_cos_th-1:], surf.g_th[0,:l_cos_th-1:], color='red', label=f'v_rot = {v_rot} Hz')
        ...
        # print(body.Omega)
        print(np.max(surf.omega_model/(2*PI)))
        ax.plot(grid.cos_th[0,:l_cos_th-1:], surf.g_th[0,:l_cos_th-1:], color='green', label=f'v_rot = {v_rot} Hz')
...
class BaseSpectra(Experiment):
    def __init__(self, name, **param):
        super().__init__(name, **param)
        # self.const_rel = Const(w_func="const", w_par = 0.1, flux_key=2, th_star = [45])
        self.const = Const(w_func="vkpower-2.0", flux_key = 'rel',  w_par = 0.8, th_star=[26.7])
    ...
    def do_experiment(self,
        ...
        ax.loglog(shot.E_null/shot.E_null.unit, shot.B_real/shot.B_real.unit/10**36, linestyle='dashed', color='blue', label='deluted BB')
        ...
        ax.loglog(shot.E_null/shot.E_null.unit, shot.B_real/shot.B_real.unit/10**36, color='blue', label='sun chemistry')
...
# A = [0.2247487  0.15114071 0.11379004 0.09147034 0.0768209  0.06661747
#      0.059228   0.0537407  0.04960815 0.04648318 0.04413614 0.04241066
#      0.04119909 0.04042878 0.1 ]
# B = [0.22612424 0.15117826 0.11387825 0.09209222 0.07814967 0.06869352
#      0.06202624 0.05720102 0.05365145 0.05102113 0.04907771 0.04766649
#      0.04668404 0.04606277 0.1       ]