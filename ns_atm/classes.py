...
class NS_SurfaceNew(Surface):
    def __init__(self, NS, size, rng):
        ...
        self.flux, self.T_eff, self.N_model = T_flux_eff(self.NS.flux_key, self.NS.flux, self.NS.T_eff, self.Flux_edd_real, self.NS.N_model)
        ...
        self.Lum = 4.0 * PI * np.sum(self.B_real*self.dE[0,0,:])

        self.lum = self.Lum / self.NS.Lum_obs
...
class NeurtonStarNew:
    def __init__(self, name="J0000+0000", sys="base", chem="s1",rel=False, 
                 ...
                 r_ns=12.0, m_ns=1.5, v_rot=700.0, i_ang=60.0, lum=0.1, n_model=N_MODEL):
        # key parameters
        self.name = name
        self.sys = sys
        self.chem = chem
    ...
    def init_grid(self, size, rng, unnull=True):
        ...
        self.N_model = self.Surface.N_model

    def calc(self, n_phi=10, n_theta=10, n_nu=10, rng_phi=(0, 360), rng_theta=(0, 180), rng_erg=(0.1, 50.0), unnull=True, zsch_key=False):
        self.init_paramters()
        if zsch_key:
            zsch = self.zsch
            rng_erg = rng_erg[0]*zsch, rng_erg[1]*zsch
